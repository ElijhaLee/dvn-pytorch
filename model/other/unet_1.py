...
class UNet(nn.Module):
    def __init__(self, useBN=False):
        ...
        self.conv1 = add_conv_stage(3, 32, useBN=False)  # changed
    ...
    def forward(self, x):   #256,3
        conv1_out = self.conv1(x)   # 256,32
        # return self.upsample21(conv1_out)
        conv2_out = self.conv2(self.max_pool(conv1_out))#128,64
        conv3_out = self.conv3(self.max_pool(conv2_out))#64,128
        conv4_out = self.conv4(self.max_pool(conv3_out))#32,256
        conv5_out = self.conv5(self.max_pool(conv4_out))#16,512
